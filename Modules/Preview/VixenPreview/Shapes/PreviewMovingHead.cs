...
using VixenModules.OutputFilter.ColorWheelFilter.Filters;
	...
	public partial class PreviewMovingHead : PreviewBaseShape, IDrawStaticPreviewShape, IDisposable, IOpenGLMovingHeadShape
	{
		...
		private void CreateIntentHandler()
		{
			...
			for (int i = 0; i < dataFlow.Outputs.Length; i++)
			{
				...
				///foreach (IDataFlowComponent colorWheelFilter in children.Where(filter =>
				//	filter is ColorWheelFilterModule))
				//{
				//	_intentHandler.ConvertColorIntentsIntoColorWheel |=
				//		((ColorWheelFilterModule) colorWheelFilter).ConvertRGBIntoIndexCommands;
				//}

				foreach (IDataFlowComponent shutterFilter in children.Where(filter => filter is ShutterFilterModule))
				{
					_intentHandler.ConvertColorIntentsIntoShutter |=
						((ShutterFilterModule) shutterFilter).ConvertRGBIntoShutterIntents;
				}

				foreach (IDataFlowComponent dimmingFilter in children.Where(filter => filter is DimmingFilterModule))
				{
					_intentHandler.ConvertColorIntentsIntoDimmer =
						((DimmingFilterModule) dimmingFilter).ConvertRGBIntoDimmingIntents;
				}
			}
		}
		...
		public void Initialize(int referenceHeight)
		{
			...
			_movingHeadOpenGL.Initialize(CalculateDrawingLength(), referenceHeight, (100.0 - BeamTransparency) / 100.0);
